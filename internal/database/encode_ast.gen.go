...
// Fields of ast_FuncType: Func TypeParams Params Results
...
// Fields of ast_RangeStmt: For Key Value TokPos Tok Range X Body
...
// Fields of ast_TypeSpec: Doc Name TypeParams Assign Type Comment
...
func encode_internal_Package(e *codec.Encoder, x *internal.Package) {
	...
	if x.Files != nil {
		e.EncodeUint(0)
		encode_slice_internal_File(e, x.Files)
	}
	e.EndStruct()
}
...
func encode_slice_internal_File(e *codec.Encoder, s []*internal.File) {
	if s == nil {
		e.EncodeNil()
		return
	}
	e.StartList(len(s))
	for _, x := range s {
		encode_internal_File(e, x)
	}
}

func decode_slice_internal_File(d *codec.Decoder, p *[]*internal.File) {
	n := d.StartList()
	if n < 0 {
		return
	}
	s := make([]*internal.File, n)
	for i := 0; i < n; i++ {
		decode_internal_File(d, &s[i])
	}
	*p = s
}

func init() {
	codec.Register([]*internal.File(nil),
		func(e *codec.Encoder, x any) { encode_slice_internal_File(e, x.([]*internal.File)) },
		func(d *codec.Decoder) any { var x []*internal.File; decode_slice_internal_File(d, &x); return x })
}

// Fields of internal_File: Name AST

func encode_internal_File(e *codec.Encoder, x *internal.File) {
	if !e.StartStruct(x == nil, x) {
		return
	}
	if x.Name != "" {
...
func decode_internal_File(d *codec.Decoder, p **internal.File) {
	proceed, ref := d.StartStruct()
	if !proceed {
		return
	}
	...
	for {
		...
		default:
			d.UnknownField("internal.File", n)
		}
		*p = &x
	}
}
...
func init() {
	codec.Register(&internal.File{},
		func(e *codec.Encoder, x any) { encode_internal_File(e, x.(*internal.File)) },
		func(d *codec.Decoder) any {
			var x *internal.File
			decode_internal_File(d, &x)
			return x
		})
}

// Fields of ast_File: Doc Package Name Decls FileStart FileEnd Scope Imports Unresolved Comments
